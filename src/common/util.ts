 ...
 * 常用公共方法
 */
import type { Util, SelectOption } from '~/common'
...
const $util = {
  /**
    ...
    */
  storage (
    type: Util.StorageType,
    action: Util.StorageAction,
    key: string,
    ...args: Util.StorageArgs
  ) {
    const [value] = args
    if (type === 'local') {
      if (action === 'set') {
  ...
  desen (str: string | number, startLen: number, endLen: number) {
    ...
    return `${toStr.substring(0, startLen)}${xin}${toStr.substring(
       toStr.length - endLen
     )}`
  },
  /**
  ...
  findTreeObj (
    targetId: string,
    options?: Util.FindTreeObjOptions
  ): Util.FindTreeObjReturn {
  ...
  /**
    ...
    */
  findTreeIds (
    targetId: Util.FindTreeIdsId,
    options?: Util.FindTreeIdsOptions
  ): Util.FindTreeIdsReturn {
    let findIds: string | undefined
    const idkey = options?.id || 'id'
    const childrenKey = options?.children || 'children'
    return function finder (
      items: Array<Util.FindTreeIdsItem>,
      parentId?: Util.FindTreeIdsId | undefined
    ) {
      for (const item of items) {
